...
from flask import Flask, redirect, render_template,url_for , request, flash , g, redirect, session
import sqlite3
import datetime
...
app_info = {}
app_info['db_file'] = r'C:\Users\karol\Desktop\FLASK\4.Sesja użytkownika i samodzielna implementacja usprawnień\data\cantor.db'
...
#Sprawdź czy taki użytkownik istnieje
    db = get_db()
    ...
    if active_admins !=None and active_admins['cnt']>0:
        flash('Applications is already set-up. Nothing to do')
...
def exchange():
    ...
    if request.method == 'GET':
        return render_template('exchange.html',offer=offer, active_menu='exchange', login=login)
        ...
        return render_template('exchange_result.html',active_menu='exchange',
         currency=currency, amount=amount, currency_info=offer.get_by_code(currency), login = login)
...
def delete_transaction(transaction_id):
    ...
    return redirect(url_for('history'), login = login)
